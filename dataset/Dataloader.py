...
def collate_fn_(batch, data_cfg, is_train, vocab, name2keypoint, word_emb_tab):
    outputs = {
        ...
        "word_embs": (
            None
            if word_emb_tab is None
            else torch.stack(
                [torch.from_numpy(word_emb_tab[sample["label"]]) for sample in batch],
                dim=0,
            )
        ),
    ...
    index_setting = data_cfg["transform_cfg"].get(
...
def build_dataloader(cfg, split, is_train=True):
    ...
    collate_func = partial(
        ...
        word_emb_tab=dataset.word_emb_tab,
    )
    if is_train:
