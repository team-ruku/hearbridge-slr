...
class S3D_two_stream_v2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pose_stream = S3D_backbone(
            ...
            freeze_block[1],
            cfg_pyramid=self.cfg_pyramid,
            coord_conv=kwargs.pop("coord_conv", None),
            use_shortcut=self.use_shortcut,
        )
        ...
        elif len(self.fusion_features) == 3 and self.fusion_features == [
            "c1",
            "c2",
            "c3",
        ]:
    ...
    def forward(self, x_rgb, x_pose):
        ...
        for i, (rgb_layer, pose_layer) in enumerate(
            zip(self.rgb_stream.backbone.base, self.pose_stream.backbone.base)
        ):
            ...
            if i in self.fuse_idx:
                ...
                if (
                    self.flag_lateral[0]
                    and "p" not in self.fusion_features[self.fuse_idx.index(i)]
                ):
                    # print(self.fusion_features[self.fuse_idx.index(i)])
                    _, x_rgb_fused = self.rgb_stream_lateral[self.fuse_idx.index(i)](
                        x_rgb, x_pose
                    )
                if (
                    self.flag_lateral[1]
                    ...
                    _, x_pose_fused = self.pose_stream_lateral[self.fuse_idx.index(i)](
                        x_rgb, x_pose
                    )
        ...
        return {
            "rgb_fused": rgb_fused,
            "pose_fused": pose_fused,
            "rgb_fea_lst": rgb_fea_lst[diff:],
            "pose_fea_lst": pose_fea_lst[diff:],
        }
