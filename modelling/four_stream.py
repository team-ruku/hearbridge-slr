    ...
    def __init__(
        ...
        **kwargs
        ...
        self.ts_high = S3D_two_stream_v2(
            use_block, freeze_block, pose_inchannels, flag_lateral, **kwargs
        )  # high frame numbers
        self.ts_low = S3D_two_stream_v2(
            use_block, freeze_block, pose_inchannels, flag_lateral, **kwargs
        )  # low frame numbers
        ...
        for i, (
            ...
            if i in self.ts_high.fuse_idx:
                ...
                if self.flag_lateral[0] and self.flag_lateral[1]:
                    _, x_rgb_high_fused = self.ts_high.rgb_stream_lateral[
                        self.ts_high.fuse_idx.index(i)
                    ](x_rgb_high, x_pose_high)
                    _, x_pose_high_fused = self.ts_high.pose_stream_lateral[
                        self.ts_high.fuse_idx.index(i)
                    ](x_rgb_high, x_pose_high)
                ...
                # low, intra ts
                if self.flag_lateral[0] and self.flag_lateral[1]:
                    _, x_rgb_low_fused = self.ts_low.rgb_stream_lateral[
                        self.ts_low.fuse_idx.index(i)
                    ](x_rgb_low, x_pose_low)
                    _, x_pose_low_fused = self.ts_low.pose_stream_lateral[
                        self.ts_low.fuse_idx.index(i)
                    ](x_rgb_low, x_pose_low)
                ...
                if self.flag_lateral[2] and self.flag_lateral[3]:
                    _, x_rgb_low_fused = self.rgb_high2low[
                        self.ts_high.fuse_idx.index(i)
                    ](x_rgb_high, x_rgb_low)
                    _, x_rgb_high_fused = self.rgb_low2high[
                        self.ts_high.fuse_idx.index(i)
                    ](x_rgb_high, x_rgb_low)
                    x_rgb_low = x_rgb_low_fused
                    x_rgb_high = x_rgb_high_fused
                ...
                if self.flag_lateral[4] and self.flag_lateral[5]:
                    _, x_pose_low_fused = self.pose_high2low[
                        self.ts_high.fuse_idx.index(i)
                    ](x_pose_high, x_pose_low)
                    _, x_pose_high_fused = self.pose_low2high[
                        self.ts_high.fuse_idx.index(i)
                    ](x_pose_high, x_pose_low)
                    x_pose_low = x_pose_low_fused
                    x_pose_high = x_pose_high_fused
