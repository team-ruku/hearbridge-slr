...
def get_selected_indexs(
    vlen, num_frames=64, is_train=True, setting=["consecutive", "pad", "central", "pad"]
):
            ...
            if vlen > num_frames:
                if train_p == "consecutive":
                    ...
                    selected_index = selected_index[
                        :num_frames
                    ]  # to make the length equal to that of no drop
...
def pad_array(array, l_and_r):
    ...
    if left > 0:
        ...
        pad = np.tile(
            ...
            tuple([left] + [1] * (len(array.shape) - 1)),
        )
    ...
    if right > 0:
        ...
        pad = np.tile(
            np.expand_dims(pad_img, axis=0),
            tuple([right] + [1] * (len(array.shape) - 1)),
        )
...
def load_video(
    ...
    if "WLASL" in dataset_name:
        video_file = "videos/{:s}.mp4".format(name)
        path = zip_file + "@" + video_file
        video_byte = ZipReader.read(path)
        video_arrays = _load_frame_nums_to_4darray(
            video_byte, selected_index
        )  # T,H,W,3
...
def load_batch_video(
    ...
    for name, vlen, ori_vfile in zip(names, vlens, ori_video_files):
        video, selected_index, pad = load_video(
            zip_file,
            name,
            vlen,
            num_output_frames,
        ...
        if "NMFs-CSL" in dataset_name:
            video = functional.resize(video.permute(0, 3, 1, 2), [256, 256]).permute(
                0, 2, 3, 1
            )
        video /= 255
    ...
    batch_videos = torch.stack(batch_videos, dim=0).permute(
        0, 1, 4, 2, 3
