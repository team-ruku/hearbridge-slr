    ...
    if center is not None:
        ...
        for cen in center:
            if cen == "nose":
                ...
                cen_y, cen_x = (right_y + left_y) / 2.0, (right_x + left_x) / 2.0
            c_y = (coords_y - cen_y.unsqueeze(1) + map_h) / 2.0
            c_x = (coords_x - cen_x.unsqueeze(1) + map_w) / 2.0
        ...
        if kwargs.pop("rela_comb", False):
            ...
            hmap = (
                hmap.view(T, len(center) + 1, hmap_num, map_h, map_w)
                .permute(0, 2, 1, 3, 4)
                .reshape(T, hmap_num * (len(center) + 1), map_h, map_w)
            )
            # print('rela_comb')
        else:
